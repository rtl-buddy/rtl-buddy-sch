...
**The schematic canvas is deliberately not on that list.**
`SchematicCanvas` gets *geometry* from elkjs and emits the SVG itself,
so every stroke and fill is a `var(--…)` in a scoped `<style>` — no
token is resolved at build time, nothing is baked, and a theme flip
needs no re-layout (the browser re-resolves the variables in place).
That is the reason the mode exists as its own component rather than a
third DOT string: a picture the SPA owns is one the stylesheet can
actually reach.
